...
@section('title', __('Tavus Settings'))
@section('titlebar_actions', '')
@section('titlebar_subtitle', __('This API key is used for these features: AI Replica'))
...
@section('settings')
    ...
    <form
        id="settings_form"
        method="POST"
        action="{{route("dashboard.admin.settings.tavus.save")}}"
    >
        @csrf
        <x-card
        ...
        >
            @if ($app_is_demo)
                <div class="mb-3">
                    <label class="form-label">{{ __('Tavus API Key') }}</label>
                    <input
                        class="form-control"
                        id="tavus_api_key"
                        type="text"
            ...
            @else
                <div
                    ...
                    <label class="form-label">{{ __('Tavus API Key') }}
                        <x-alert class="mt-2">
                            <x-button
                                variant="link"
                    ...
                    <x-alert class="mt-2">
                        <p>
                            {{ __('Please ensure that your Tavus api key is fully functional and billing defined on your Tavus account.') }}
                        </p>
                    </x-alert>
                    <a
                        class="btn btn-primary mb-2 mt-2 w-full"
                        href="{{ route('dashboard.admin.settings.tavus.test') }}"
                        target="_blank"
                    >
